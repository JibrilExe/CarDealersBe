...
def write_engine(cylinders, name):
    ...
    with open(filepath, "w") as f:
        ...
        for i in range(cylinders):
            f.write(f"    output wire{i+1}: ignition_wire();\n")
        f.write(code2)
        for i in range(cylinders):
            f.write(f"    rod_journal rj{i}(angle: {(360.0/cylinders)*i:.1f} * units.deg)\n")
        f.write("    c0\n")
        for i in range(cylinders):
            f.write(f"        .add_rod_journal(rj{i})\n")
        f.write(code3)
        for i in range(cylinders):
            f.write(f'''        .add_cylinder(
            piston: piston(piston_params, blowby: k_28inH2O(0.001)),
            connecting_rod: connecting_rod(cr_params),
            rod_journal: rj{i},
            intake: intake,
            exhaust_system: exhaust0,
            ignition_wire: wires.wire{i+1},
